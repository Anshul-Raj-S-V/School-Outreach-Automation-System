...
Cold outreach campaigns naturally have low response rates. Running every email through a paid AI API adds cost for no real benefit when the bottleneck isn't email quality — it's getting a clean, validated list and reliably delivering to it. 
This script handles the whole pipeline locally in Python: validate the data, personalize the message, attach the brochure, send with rate limiting, and log everything so the campaign can be safely resumed.
...
1.Your contact list as `UWR_School_Outreach_List.csv`
2.Your brochure as a PDF
3.Set your sender credentials as environment variables (never hardcode them):
```bash
